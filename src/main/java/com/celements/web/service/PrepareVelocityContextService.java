...
public class PrepareVelocityContextService implements IPrepareVelocityContext {
  ...
  @Requirement("all")
  private ConfigurationSource allConfigSource;
  ...
  public void prepareVelocityContext(VelocityContext vcontext) {
    ...
    LOGGER.debug("prepareVelocityContext [" + count + "]: with vcontext [" + vcontext + "].");
    ...
    LOGGER.trace("prepareVelocityContext [" + count + "]: after fixTdocForInvalidLanguage.");
    ...
    if (LOGGER.isInfoEnabled()) {
      ...
      if (getExecContext().getProperty(_CEL_PREPARE_VELOCITY_TOTALTIME) != null) {
        totalTimeUsed = (Long) getExecContext().getProperty(_CEL_PREPARE_VELOCITY_TOTALTIME);
      }
      totalTimeUsed += timeUsed;
      getExecContext().setProperty(_CEL_PREPARE_VELOCITY_TOTALTIME, totalTimeUsed);
      LOGGER.info("prepareVelocityContext [" + count + "]: with vcontext [" + vcontext
          + "] finished in [" + timeUsed + "], total time [" + totalTimeUsed + "].");
    }
  }
  ...
  void initPanelsVelocity(VelocityContext vcontext) {
    if (vcontext != null) {
      ...
      LOGGER.debug("leftPanels [" + vcontext.get("leftPanels") + "] and rightPanels ["
          + vcontext.get("rightPanels") + "] after initPanelsVelocity.");
    }
  }
  ...
  void initCelementsVelocity(VelocityContext vcontext) {
    ...
    if ((vcontext != null) && (getContext().getWiki() != null)) {
      if (!vcontext.containsKey(getVelocityName())) {
        vcontext.put(getVelocityName(),
            getContext().getWiki().getPluginApi(getVelocityName(), getContext()));
      }
      ...
      if (!vcontext.containsKey("skin_doc") && (baseRef != null)) {
        try {
          DocumentReference skinDocRef = modelUtils.resolveRef(
              getContext().getWiki().getSkin(getContext()), DocumentReference.class, baseRef);
          Document skinDoc = getContext().getWiki().getDocument(skinDocRef, getContext())
      ...
      if (!vcontext.containsKey("admin_language")) {
        ...
        LOGGER.debug("added admin_language to vcontext: " + webUtilsService.getAdminLanguage());
      }
      ...
      try {
        if (!vcontext.containsKey("celements2_baseurl") && (getContext().getURLFactory() != null)) {
          ...
          if (celements2_baseurl.indexOf("/", 8) > 0) {
            vcontext.put("celements2_baseurl",
                celements2_baseurl.substring(0, celements2_baseurl.indexOf("/", 8)));
          }
        }
      } catch (XWikiException exp) {
      ...
      if (!vcontext.containsKey("page_type")) {
        ...
        LOGGER.debug("initCelementsVelocity: page_type [{}] already available in context",
      ...
      if (!vcontext.containsKey("isContentEditor")) {
        vcontext.put("isContentEditor",
            getContext().getWiki().getUser(getContext().getUser(), getContext())
      ...
      if (!vcontext.containsKey("tinyMCE_height")) {
        vcontext.put("tinyMCE_height", getRTEheight());
      }
    }
  }
  ...
  String getRTEwidth(XWikiContext context) {
    ...
    if (getCelementsSkinDoc(context) != null) {
      BaseObject pageTypeObj = new PageTypeCommand().getPageTypeObj(context.getDoc(), context)
          .getPageTypeProperties(context);
      if (pageTypeObj != null) {
    ...
    return Integer.toString(tinyMCEwidth);
  }

  String getRTEheight() {
    int tinyMCEheight = -1;
    String tinyMCEheightStr = "";
    if (getCelementsSkinDoc(getContext()) != null) {
      BaseObject pageTypeObj = pageTypeResolver.getPageTypeObject(getContext().getDoc());
      if (pageTypeObj != null) {
        tinyMCEheight = pageTypeObj.getIntValue("rte_height", -1);
        tinyMCEheightStr = Integer.toString(tinyMCEheight);
      }
      if (tinyMCEheight <= 0) {
        tinyMCEheightStr = allConfigSource.getProperty("editbox_height");
        if (!Strings.isNullOrEmpty(tinyMCEheightStr)) {
          try {
            tinyMCEheight = Integer.parseInt(tinyMCEheightStr);
          } catch (NumberFormatException nfExp) {
            LOGGER.info("Failed to parse integer editbox_height {}", tinyMCEheightStr);
          }
        }
      }
    }
    ...
    return Integer.toString(tinyMCEheight);
  }
  ...
  private Document getCelementsSkinDoc(XWikiContext context) {
    ...
    try {
      ...
      LOGGER.error("Failed to load celements2_skin" + " (celements2web:XWiki.Celements2Skin) ",
          exp);
    }
    return skinDoc;
  }

  private XWikiDocument getCelementsSkinXWikiDoc(XWikiContext context) throws XWikiException {
    return context.getWiki()
        .getDocument(new DocumentReference("celements2web", "XWiki", "Celements2Skin"), context);
  }
  ...
  boolean showPanelByConfigName(XWikiContext context, String configName) {
    if (isPageShowPanelOverwrite(configName, context.getDoc())) {
      return (1 == getPagePanelObj(configName, context.getDoc()).getIntValue("show_panels"));
      ...
      boolean showPanels = ("1".equals(context.getWiki().getXWikiPreference(configName, context)));
      LOGGER.debug("using globalPref for panels " + configName + " -> " + showPanels);
      return showPanels;
    } else if (context.getWiki() != null) {
      boolean showPanels = ("1".equals(context.getWiki().getSpacePreference(configName, context)));
    ...
    if (context.getDoc() != null) {
      ...
      if (pTRefForCurrDoc != null) {
        try {
          ...
          XWikiDocument pageTypeDoc = new PageType(pageTypeDocRef)
  ...
  private boolean isPageShowPanelOverwrite(String configName, XWikiDocument theDoc) {
    try {
      return ((getPagePanelObj(configName, theDoc) != null)
          && (((BaseProperty) getPagePanelObj(configName, theDoc).get("show_panels"))
              .getValue() != null));
    } catch (XWikiException exp) {
      LOGGER.error("", exp);
      return false;
  ...
  private boolean isSpaceOverwrite(XWikiContext context) {
    return ((getSpaceOverwrite(context) != null) && !"".equals(getSpaceOverwrite(context)));
  }
  ...
  private String getPanelString(XWikiContext context, String configName) {
    ...
    if (isGlobalPref(context)) {
      ...
      panelsString = getPagePanelObj(configName, context.getDoc()).getStringValue("panels");
    } else if ((getPageTypeDoc(context) != null)
        && isPagePanelsOverwrite(configName, getPageTypeDoc(context))) {
      panelsString = getPagePanelObj(configName, getPageTypeDoc(context)).getStringValue("panels");
    } else if (isSpaceOverwrite(context)) {
      panelsString = context.getWiki().getSpacePreference(configName, getSpaceOverwrite(context),
          "", context);
    } else {
  ...
  private boolean isPagePanelsOverwrite(String configName, XWikiDocument theDoc) {
    return ((getPagePanelObj(configName, theDoc) != null)
        && (!"".equals(getPagePanelObj(configName, theDoc).getStringValue("panels"))));
  }
   ...
   * preferences. If the wiki is multilingual try to get the language passed in the
   * request. If none was passed try to get it from a cookie. If no language cookie exists
  ...
  String getLanguagePreference(XWikiContext context) {
    ...
    if (!context.getWiki().isMultiLingual(context)) {
      LOGGER.debug(
          "getLanguagePreference: isMultiLingual [" + context.getWiki().isMultiLingual(context)
              + "] defaultLanguage [" + webUtilsService.getDefaultLanguage() + "].");
  ...
  private String getLanguageFromAcceptedHeaderLanguages() {
    ...
    if ((acceptHeader != null) && (!acceptHeader.equals(""))) {
      ...
      LOGGER.debug(
          "getLanguageFromAcceptedHeaderLanguages: getAcceptedLanguages " + acceptedLanguages);
      ...
      if (xwiki.Param("xwiki.language.forceSupported", "0").equals("1")) {
        ...
        LOGGER.debug("getLanguageFromAcceptedHeaderLanguages: forceSupported lang " + " languages ["
            + Arrays.deepToString(available.toArray(new String[] {})) + "].");
        // Filter only configured languages
        acceptedLanguages.retainAll(available);
      }
      LOGGER.debug(
          "getLanguageFromAcceptedHeaderLanguages: acceptedLanguages after " + acceptedLanguages);
  ...
  private boolean isConsiderBrowserAcceptLanguages() {
    ...
    return !xwiki.Param("xwiki.language.preferDefault", "0").equals("1")
        && !xwiki.getSpacePreference("preferDefaultLanguage", "0", getContext()).equals("1");
  }
  ...
  private String getLanguageFromUserPreferences() throws XWikiException {
    ...
    userdoc = getContext().getWiki()
        .getDocument(modelUtils.resolveRef(userFN, DocumentReference.class), getContext());
    if (userdoc != null) {
      language = Util
  ...
  private String getLanguageFromCookie() {
    ...
    return Util.normalizeLanguage(
        getContext().getWiki().getUserPreferenceFromCookie("language", getContext()));
  }
  ...
  private String getLanguageInRequestParam() {
    String language = Util.normalizeLanguage(getContext().getRequest().getParameter("language"));
    if ((language != null) && (!language.equals(""))) {
      Object cookieAddedBefore = getExecContext().getProperty(ADD_LANGUAGE_COOKIE_DONE);
  ...
  boolean isInvalidLanguageOrDefault(String language) {
    return language.equals("default")
        || (isSuppressInvalid() && !webUtilsService.getAllowedLanguages().contains(language));
  }
  ...
  /**
   * Construct a list of language codes (ISO 639-1) from the Accept-Languages header. This
  ...
  void fixTdocForInvalidLanguage(VelocityContext vcontext) {
    ...
    if ((doc != null) && (vcontext != null)) {
      ...
      if (isTdocLanguageWrong(vTdocBefore)) {
        ...
        LOGGER.debug("skip fixTdocForInvalidLanguage because vTdoc launguage is" + " correct.");
      }
    } else {
      LOGGER.debug("skip fixTdocForInvalidLanguage doc [" + doc + "] vcontext [" + vcontext + "].");
    }
  }
