...
public class FileBaseTag0 implements IMandatoryDocumentRole {
  ...
  public void checkDocuments() throws XWikiException {
    LOGGER.trace("Start checkDocuments in FileBaseTag0 for database ["
          + getContext().getDatabase() + "].");
    if (noMainWiki() && !isSkipCelementsFileBaseTag0()) {
      LOGGER.trace("before checkFileBaseTag0 for database ["
          + getContext().getDatabase() + "].");
      checkFileBaseTag0();
    } else {
      LOGGER.info("skip mandatory checkFileBaseTag0 for database ["
          + getContext().getDatabase() + "], noMainWiki [" + noMainWiki()
          + "], skipCelementsParam [" + isSkipCelementsFileBaseTag0() + "].");
    }
    LOGGER.trace("end checkDocuments in FileBaseTag0 for database ["
        + getContext().getDatabase() + "].");
  }

  boolean isSkipCelementsFileBaseTag0() {
    boolean isSkip = getContext().getWiki().ParamAsLong(
        "celements.mandatory.skipFileBaseTag0", 0) == 1L;
    LOGGER.trace("skipFileBaseTag0 for database ["
  ...
  void checkFileBaseTag0() throws XWikiException {
    DocumentReference fileBaseTag0Ref = getFileBaseTag0Ref(
        getContext().getDatabase());
    ...
    if (!getContext().getWiki().exists(fileBaseTag0Ref, getContext())) {
      ...
      LOGGER.trace("FileBaseTag0Document already exists. ["
          + getContext().getDatabase() + "]");
    }
  ...
  boolean checkPageType(XWikiDocument fileBaseTag0Doc) throws XWikiException {
    DocumentReference pageTypeClassRef = getPageTypeClasses().getPageTypeClassRef(
        getContext().getDatabase());
    BaseObject pageTypeObj = fileBaseTag0Doc.getXObject(pageTypeClassRef, false,
        getContext());
    if (pageTypeObj == null) {
