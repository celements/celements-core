...
public abstract class AbstractDocumentConfigurationSource
  ...
  public boolean containsKey(String key) {
    // Since a single XObject holds all the properties we need to be careful
    // here, overriding one property will put
    // all the default keys in the source. To determine that the source contains
  ...
  public List<String> getKeys() {
    // TODO: introduce a
    // DocumentAccessBridge#getXClassPropertyNames(DocumentReference) to
  ...
  public <T> T getProperty(String key, Class<T> valueClass) {
    ...
    // Make sure we don't return null values for List and Properties (they must
  ...
  private DocumentReference getFailsafeDocumentReference() {
    ...
    try {
      ...
      // We verify that no error has happened and if one happened then we skip
      // this configuration source. This
      // ensures the system will continue to work even if this source has a
      // problem.
