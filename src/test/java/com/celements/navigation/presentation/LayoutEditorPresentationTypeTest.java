...
public class LayoutEditorPresentationTypeTest extends AbstractComponentTest {
  ...
  public void setUp_DefaultPresentationTypeTest() throws Exception {
    ...
    layoutEditorPresType = (LayoutEditorPresentationType) Utils
        .getComponent(IPresentationTypeRole.class, "layoutEditor");
    ...
    expect(xwiki.getDocument(eq("MySpace.MyCurrentDoc"), same(context))).andReturn(currentDoc)
        .anyTimes();
  }
  ...
  public void testAppendMenuItemLink() throws Exception {
    ...
    assertEquals("<a href=\"/MySpace/MyCurrentDoc\""
        ...
        + " id=\"N1:MySpace:MySpace.MyCurrentDoc\">My Current Doc</a>", outStream.toString());
    verifyDefault();
  }
  ...
  public void testAppendMenuItemLink_withCellId() throws Exception {
    ...
    cellPropertiesObj.setXClassReference(new CellsClasses().getCellClassRef(context.getDatabase()));
  ...
  public void testAppendMenuItemLink_use_navImages() throws Exception {
    ...
    expect(menuNameCmdMock.getMultilingualMenuName(eq(currentDoc.getFullName()), eq("de"),
        same(context))).andReturn(menuName).atLeastOnce();
    expect(menuNameCmdMock.addNavImageStyle(eq(currentDoc.getFullName()), eq("de"), same(context)))
        .andReturn("style=\"background-image:url(abc);\"").atLeastOnce();
    expect(menuNameCmdMock.addToolTip(eq(currentDoc.getFullName()), eq("de"), same(context)))
        .andReturn("").atLeastOnce();
    navFilterMock.setMenuPart(eq(""));
    expectLastCall().atLeastOnce();
  ...
  public void testAppendMenuItemLink_noLink() throws Exception {
    ...
    assertEquals(
        "<span class=\"cel_cm_navigation_menuitem first last cel_nav_odd"
